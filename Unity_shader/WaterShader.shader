...
Category {
	...
	uniform sampler2D _MainTex, _WaterMap, _SecondaryRefractionTex, _AnisoMap ,_CausticsAnimationTexture, _DUDVFoamMap,  _FoamDiffuse;
	uniform float4 _MainTex_ST , _WaterMap_ST, _SecondaryRefractionTex_ST, _AnisoMap_ST ,_CausticsAnimationTexture_ST, _DUDVFoamMap_ST;
	...
	inline half CalculateCaustics(float2 uv, half waterAttenuationValue, fixed4 waterMapValue) 
	{
		...
		return (causticsAnimationColorChannel.x * causticsFrame.r
				...
				+ causticsAnimationColorChannel.z * causticsFrame.b) * waterMapValue.g *_CausticsStrength * (1.0 - waterAttenuationValue);
	}	  
	...
	inline fixed3 CalculateRefraction(float2 uv_Caustics, half refrStrength, float2 uv_SecondaryRefrTex,												
	...
	{	
		...
		refractionColor += CalculateCaustics(uv_Caustics + dudvValue, waterAttenuationValue, waterMapValue);
	...
	inline fixed3 CombineEffectsWithLighting( fixed3 refraction, half refrStrength,	fixed3 reflection, fixed3 pNormal,
												fixed3 normViewDir,	fixed3 normLightDir, half2 uv_MainTex, half2 uv_Aniso, half waterAttenuationValue,
	...
	fixed4 frag (v2f i) : COLOR
	{
	    ...
	    // Refractivity		
		fixed3 refrColor = CalculateRefraction(	i.uv_MainTex, waterMapValue.a, i.uv_SecondaryRefrTex,
		...
		outColor.rgb = CombineEffectsWithLighting(refrColor, waterMapValue.a, reflectCol, pNormal, normViewDir,
												 i.lightDir, i.uv_MainTex, i.uv_Aniso,  waterAttenuationValue, foam, mask, foamAmount, foamValue, i.lightDir );
