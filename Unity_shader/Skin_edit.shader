Shader "Easy Skin Shading/Skin_edit" {
	Properties {
	...
	SubShader {
		Pass {
			...
			v2f vert (appdata_tan v)
			{
				...
				o.normalDir = mul(UNITY_MATRIX_MVP, v.vertex);
			...
			float4 frag (v2f i) : SV_TARGET
			{
				...
				if(_WorldSpaceLightPos0.w == 0)
				{
					...
					distance = length(fragmentToLightSource);
				...
				float3 diffColor = diffuseWeight * (atten * (_LightColor0.rgb / (distance * distance)));
				...
				float3 litColor = (diffColor + subsurfaceColor * saturate(dot(NF, lightDirection))) + pow(specColor , (1- _SpecularMult) ) + (UNITY_LIGHTMODEL_AMBIENT.rgb *0.5) + (ReflectProbeColor.xyz * 0.025);	// EDIT! + specColor : スペキュラー制御
		...
		Pass {
			Tags { "RenderType" = "Opaque" "IgnoreProjector" = "True"  "LightMode" = "ForwardAdd" }
