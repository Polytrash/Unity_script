	...
	SubShader {
		Pass {
			...
			)
			{
				...
				// EDIT! float2(ndoth, m)).x : 暗部の明度調整(フレネル制御)、パラメータ反転
			...
			float4 frag (v2f i) : SV_TARGET
			{
				...
				float3 litColor = (diffColor + subsurfaceColor) * LIGHT_ATTENUATION(i) + pow(specColor , (1- _SpecularMult)) + UNITY_LIGHTMODEL_AMBIENT.rgb + ReflectProbeColor.xyz;	// EDIT! + specColor : スペキュラー制御
