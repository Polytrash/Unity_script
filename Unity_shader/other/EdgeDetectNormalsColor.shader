...
//----------------------------------------------------------------------------//
	...
	fixed4 fragLum (v2flum i) : SV_Target
	{
		...
		half len = dot(diff, diff) * ((i.uv[1].x + step(.1,min(.1,sin(_Time.w -1) ))) * (i.uv[2].y + step(.1,min(.1,cos(_Time.w + 1) ))))  ;
		len = step(len, _Threshold);


 
		float4 blendColor = (0,0,0,1);

			
		blendColor = lerp(original, _Color, 1-len);
		return blendColor;			
	}	
	...
	half4 fragRobert(v2f i) : SV_Target {				
		...
		// ノイズブレンド追加
		float n = noise(i.pos.xyz * _NoiseFactor);
		half4 blend =	lerp(tex2D(_MainTex, i.uv[0].xy), _BgColor, _BgFade);
		if(n < 0.4) discard	 ;
		if(edge > 0)	 
    		 return blend;	
		 else
